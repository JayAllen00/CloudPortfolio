resource "aws_security_group" "forECS" {
  ...
  ingress {
    ...
    to_port          = 80
    protocol         = "tcp"
    #security_groups  = ["${aws_security_group.PowerfullALB.id}"]
...
resource "aws_ecs_task_definition" "service" {
  ...
  container_definitions    = jsonencode([
    {
      name      = "catpipeline"
      image     = "Your_Account_ID.dkr.ecr.us-east-2.amazonaws.com/catpipeline:latest"
      #vcpu       = 0.5
      #memory    = 1
      essential = true
...
resource "aws_security_group" "PowerfullALB" {
  name        = "PowerfullALB"
  description = "Allow HTTP inbound traffic"
  vpc_id      = data.aws_vpc.default_vpc.id
...
#  certificate_arn   = "arn:aws:iam::187416307246:server-certificate/test_cert_rab3wuqwgja25ct3n4jdj2tzu2"
